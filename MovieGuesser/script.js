...
next.addEventListener('click', (e) =>{
  ...
  desc.innerHTML = movies[index].explanation;

 hint.addEventListener('click', (e) => {
    /*const randomNumber = Math.round(Math.random()* 10 + 1);*/
    let index = randomNumber;
    const hintelem = document.createElement('div');
    hintelem.classList.add('alert-secondary');
    output.innerHTML = '';
  ...
  if (input.value == movies[index].title) {
    elem.classList.add('alert-success');
    elem.innerHTML = 'Correct! It was ' + movies[index].title;
  } else {
    elem.classList.add('alert-danger');
    elem.innerHTML = 'Incorrect, it was ' + movies[index].title;
  }
